(() => {
  ...
  preloadImages([...TILE_URLS, WIN_URL]).catch(()=>{});
  startLevel(levelIndex);
  ...
  async function startLevel(idx){
    ...
    stabilizeBoardNoScore();
    renderAll();
    bindBoardInput();
  }

  function applyResponsiveCellSize(){
    const gap = 6, boardPadding = 20, safeMargin = 28;
  ...
  function stabilizeBoardNoScore(){
    let guard = 0;
    while(true){
      const m = findMatches();
      if(m.size === 0) break;
